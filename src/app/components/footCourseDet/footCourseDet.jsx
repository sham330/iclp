...
import ModalBooking from "../ModalBooking/ModalBooking"; // Adjust the import path as needed
...
const FootCourseDet = () => {
  ...
  // Get coursename from params - in Next.js app router, dynamic segments are directly available
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const selectedCourse = data[coursename] || null;
        if (selectedCourse) {
          selectedCourse.key = coursename;
        }
  ...
  if (loading) {
    return <div className="fcd-loading">Loading course details...</div>;
  }

  if (error) {
    return <div className="fcd-error">Error: {error}</div>;
  }

  if (!course) {
    return <div className="fcd-not-found">Course not found</div>;
  }
