"use client";

import React, { useState, useEffect, useRef } from "react";
import { useRouter } from "next/navigation";
import {
  FaInstagram,
  FaLinkedin,
  FaPhone,
  FaInfoCircle,
  FaSearch,
  FaYoutube,
} from "react-icons/fa";
import "../AppBar/appbar.css";
const AppBar = () => {
  const router = useRouter();
  const [searchTerm, setSearchTerm] = useState("");
  const [suggestions, setSuggestions] = useState([]);
  const [showSuggestions, setShowSuggestions] = useState(false);
  const [allCourses, setAllCourses] = useState([]);
  const [activeSuggestion, setActiveSuggestion] = useState(-1);
  const searchInputRef = useRef(null);
  const suggestionsRef = useRef(null);
  const debounceTimerRef = useRef(null);

  const socialLinks = {
    instagram:
      "https://www.instagram.com/iclp_technologies?igsh=MWx5ZGZqbnJoOGUwcA%3D%3D&utm_source=qr",
    linkedin: "http://linkedin.com/in/iclp-technologies-b40044354",
    youTybe: "https://youtube.com/@iclp_technologies?si=zLBwqpP0k5LAdgNg",
  };

  const phoneNumber = "86810 26181";

  // Load all course names from your JSON data
  useEffect(() => {
    const courses = [
      { name: "Java", path: "java-online-training-chennai" },
      { name: "Python", path: "python-development-online-training" },
      { name: "C Sharp", path: "c-sharp-online-training" },
      { name: "Control-M", path: "control-m-training-chennai" },
      {   path: "android-development-training-chennai", name: "Android Development" },
      { name: "R Programming", path: "r-programming-online-training-chennai" },
      { name: "JavaScript", path: "java-script-online-training-chennai" },
      { name: "Perl Scripting", path: "perl-scripting-online-training" },
      { name: "TypeScript", path: "type-script-online-training" },
      { name: "Full Stack Developer", path: "full-stack-developer-online-training" },
      { name: "Node JS", path: "node-js-online-training-chennai" },
      { name: "Hospital Administration", path: "hospital-administration-course" },
      { name: "Healthcare Analytics", path: "healthcare-analytics-course" },
      { name: "Food Technology", path: "food-technology-course" },
      { name: "React JS", path: "react-js-online-training-chennai" },
      { name: "Angular", path: "angular-developer-online-training-chennai" },
      { name: "WordPress", path: "wordpress-online-training-chennai" },
      { name: "Azure Data Factory", path: "azure-data-factory-training-chennai" },
      { name: "HTML", path: "html-developer-online-training-chennai" },
      { name: "CSS", path: "css-online-training" },
      { name: "BluePrism", path: "blue-prism-training" },
      { name: "MERN Stack", path: "mern-stack-training-chennnai" },
      { name: "RPA + Artificial Intelligence", path: "rpa-training-chennai" },
      { name: "UiPath", path: "uipath-certification-training" },
      { name: "Apache Spark Advanced", path: "apache-spark-training-chennai" },
      { name: "Linux Administrator training", path: "linux-administrator-training" },
      { name: "Palo Alto training", path: "palo-alto-training" },
      { name: "Figma advanced training", path: "figma-advanced-training-chennai" },
      { name: "Vmware Vsphere training", path: "vmware-vsphere-training-chennai" },
      { name: "JQuery", path: "j-query-developer-online-training-chennnai" },
      { name: "Full Stack Java", path: "full-stack-java-training-online-chennai" },
      { name: "Python Full Stack", path: "python-full-stack-developer-online-training-chennai" },
      { name: "Groovy and Grails", path: "groovy-and-grails-online-training" },
      { name: "Workday HCM", path: "workday-hcm-online-course-chennai" },
      { name: "Salesforce", path: "salesforce-online-training-chennai" },
      { name: "Salesforce Developer", path: "salesforce-developer-training" },
      { name: "Azure DevOps", path: "azure-devops-training-chennai" },
      {name:"AWS DevOps",path:"aws-devops-training-chennai"},
      { name: "Tableau", path: "tableau-certification-training-chennai" },
      { name: "Motion Graphics Training", path: "motion-graphics-training" },
      { name: "Penetration Testing", path: "advanced-pentest-certification-training-chennai" },
      { name: "Network Security", path: "system-network-security-training-chennai" },
      { name: "CEH Preparation", path: "ceh-advanced-training-chennai" },
      { name: "Cloud Security", path: "ccsk-foundation-training-chennai" },
      { name: "Docker", path: "docker-training-chennai" },
      { name: "SOC Analyst", path: "soc-analyst-csa-training-chennai" },
      { name: "Git & GitHub", path: "git-github-training-chennai" },
      { name: "Terraform", path: "terraform-training-chennai" },
      { name: "Dotnet Fullstack", path: "dotnet-fullstack-training" },
      { name: "Kubernetes Training", path: "kubernetes-training-chennai" },
      { name: "HR Analytics Training", path: "hr-analytics-training" },
      { name: "Oracle Fusion Financials", path: "oracle-fusion-financials" },
      { name: "Kofax Online Training", path: "kofax-totalagility-training" },
      { name: "Video Editing Training", path: "video-editing-training" },
      { name: "Mobile App development", path: "mobile-app-development-certification-course-training-chennai" },
      {name:"RAG Systems",path:"rag-systems-ai-certification-course-chennai"},
      { name: "Data Steward Training", path: "data-steward-training" },
      { name: "Snowflake Training", path: "snowflake-certification-training-chennai" },
      { name: "Databricks", path: "databricks-spark-certification-training-chennai" },
      { name: "SAP FICO Consultant", path: "sap-fico-consultant-training" },
      { name: "Hardware Network training", path: "hardware-networking-training" },
      { name: "AutoCAD", path: "autocad-training" },
      { name: "PLC & SCADA", path: "plc-scada-training" },
      {name:"Informatica Data Governance",path: "informatica-data-governance-course",},
      { name: "Cyber Law", path: "cyber-law-training" },
      { name: "Postgresql", path: "postgresql-admin-online-training-chennai" },
      { name: "Medical Coding", path: "medical-coding-cpc-course" },
      {  path: "ibm-maximo-training-chennai", name: "IBM Maximo Training" },
      {  path: "salesforce-developer-training-chennai", name: "Salesforce Developer Training" },
      { name: "Appian ", path: "appian-online-training" },
      { name: "ASP .Net", path: "asp-net-course-training-chennai" },
      { name: "Basics of Automation Testing", path: "automation-testing-online-course-chennai" },
      { name: "Linux", path: "linux-certification-training-chennai" },
      { name: "AI for Automation", path: "ai-for-automation-course-chennai" },
      { name: "API Testing (Postman Basics)", path: "api-testing-certification-training-chennai" },
      { name: "Bootstrap", path: "bootstrap-course-training-chennai" },
      { name: "OOPs", path: "oops-concepts-training-chennnai" },
      { name: "DevOps", path: "devops-training-chennai" },
      { name: "Workday Integration Training", path: "workday-integration-training", },
      { name: "Workday Payroll Training", path: "workday-payroll-training", },
      {name:"Pega",path: "pega-online-training"},
      {
        name: "Primavera P6", path: "primavera-p6-online-training",
      },
      { name: "AWS", path: "aws-online-training-chennai" },
      { name: "Cisco UCS", path: "cisco-ucs-training" },
      { name: "Cyber Security", path: "cyber-security-online-training-chennai" },
      { name: "Ethical Hacking", path: "ethical-hacking--online-training-chennai" },
      { name: "CISM", path: "cism-certification-online-training-chennai" },
      { name: "CyberArk", path: "cyber-ark-training" },
      { name: "CISA", path: "certified-information-systems-auditor-cisa" },
      { name: "CISM (Manager)", path: "certified-information-security-manager-cism" },
      { name: "Blockchain", path: "blockchain-online-training" },
      { name: "Selenium", path: "selenium-training-chennai" },
      { name: "Manual Testing", path: "manual-testing-course-online-training-chennai" },
      { name: "JMeter", path: "jmeter-online-training-chennai" },
      { name: "Workday Studio Training", path: "workday-studio-training" },
      { name: "Software Testing", path: "software-testing-certification-course-chennai" },
      { name: "Data Analytics Course", path: "data-analytics-course-chennai" },
      { name: "Workday financial training", path: "workday-financial-training" },
      { name: "Collibra", path: "collibra-online-training-course" },
      { name: "Spring boot online training", path: "spring-boot-online-training-chennai" },
      { name: "ETL Testing", path: "etl-testing-online-training-chennai" },
      { name: "LoadRunner", path: "loadrunner-online-training-chennai" },
      { name: "Clinical Research & Pharmacovigilance Certification Course", path: "clinical-research-pharmacovigilance-certification-course" },
      {path:"hospital-administration-certification-course", name:"Hospital Administration Certification Course"},
      { name: "SoapUI", path: "soap-ui-online-training-chennai" },
      { name: "Team center PLM  training", path: "teamcenter-plm-online-training" },
      { name: "Business analyst  training", path: "business-analyst-online-training" },
      { name: "SAP treasury and risk management", path: "sap-treasury-and-risk-management-online-training" },
      { name: "Machine Learning", path: "machine-learning-online-course-chennai" },
      { name: "Data Science with Python", path: "data-science-with-python-training-chennai" },
      { name: "ML using R", path: "machine-learning-using-r-course" },
      { name: "AI & ML", path: "ai-ml-training" },
      { name: "Data Science", path: "data-science-course-online" },
      { name: "Tally Accounting", path: "tally-accounting-online-training" },
      { name: "Tally GST Training", path: "tally-gst-training" },
      { name: "Internet and Network Basics", path: "internet-and-network-basics-training-chennai" },
      { name: "Computer Fundamentals", path: "computer-fundamentals-training-chennai" },
      { name: "Basic Web designing", path: "basic-web-designing-course-chennai" },
      { name: "Programming Basics", path: "programming-basics-training" },
      { name: "Basic Graphic Designing", path: "basic-graphic-designing-course-chennai" },
      { name: "Digital Literacy", path: "digital-literacy-training" },
      { name: "AI", path: "artificial-intelligence-course-online-chennai" },
      { name: "ChatGPT", path: "chatgpt-online-course" },
      { name: "MSQL", path: "msql-online-training-chennai" },
      { name: "Soft Tech Skills", path: "soft-tech-skills-training", },
      { name: "SQL with PHP", path: "sql-with-php-online-training-chennai" },
      { name: "AR VR", path: "ar-vr-technologies-training", },
      { name: "PL/SQL", path: "pl-and-sql-online-training-chennai" },
      { name: "Excel", path: "microsoft-excel-online-training-chennai" },
      { name: "Microsoft Dynamics", path: "microsoft-dynamics-online-training" },
      { name: "Microsoft Azure", path: "microsoft-azure-online-training-chennai" },
      { name: "Excel Macros and VBA", path: "excel-macros-and-vba-online-training" },
      { name: "Digital Marketing", path: "digital-marketing-online-training-chennai" },
      {path:"certified-scrum-master-training-chennai", name:"Certified Scrum Master"},
      { name: "UI/UX", path: "ui-ux-designer-online-training-chennai" },
      { name: "CCNA", path: "ccna-online-training-chennai" },
      { name: "CCNP", path: "ccnp-online-training" },
      { name: "Fortinet", path: "fortinet-online-training" },
      { name: "SAS", path: "sas-course-online" },
      { name: "Informatica PowerCenter", path: "informatica-power-center-course" },
      { name: "Informatica ILM", path: "informatica-ilm-online-training" },
      { name: "Unix Shell Scripting", path: "unix-shell-scripting-online-training-chennai" },
      { name: "C and C++", path: "c-cpp-programming-course-training-chennai" },
      { name: "Corel Draw", path: "coreldraw-masterclass" },
      { name: "Alation Data Catalog", path: "alation-data-catalog-training", },
      { "name": "IBM DataPower", "path": "ibm-data-power" },
      { "name": "IBM Cognos", "path": "ibm-cognos" },
      { "name": "IBM Watson Analytics", "path": "ibm-watson-analytics" },
      { "name": "IBM SPSS", "path": "ibm-spss" },
      { "name": "IBM Cloud Essentials", "path": "ibm-cloud-essentials" },
      { "name": "IBM Bluemix", "path": "ibm-bluemix" },
      { "name": "IBM QRadar SIEM", "path": "ibm-qradar-siem" },
      { "name": "IBM BPM Administration", "path": "ibm-bpm-administration" },
      { "name": "IBM Sterling", "path": "ibm-sterling" },
      { "name": "WebSphere Application Server", "path": "websphere-application-server" },
      { "name": "IBM AIX", "path": "ibm-aix" },
      { "name": "IBM Mainframe Developer", "path": "ibm-mainframe-developer" },
      { "name": "IBM Tivoli", "path": "ibm-tivoli" },

      // SAP Courses
      { name: "SAP FICO", type: "sap", path: "sap-fico-training-chennai" },
      { name: "SAP MM", type: "sap", path: "sap-mm-training-chennai" },
      { name: "SAP SD", type: "sap", path: "sap-sd-training-chennai" },
      {name:"SAP WM",type:"sap",path:"sap-wm"},
      { name: "SAP Banking", type: "sap", path: "sap-banking", },
      {name:"SAP Integration Suite (CPI)",type:"sap",path:"sap-integration-suite-cpi-training-chennai"},
      {name:"SAP EWM",type:"sap", path: "sap-ewm-training-chennai"},
      { name: "SAP PP", type: "sap", path: "sap-pp-training-chennai" },
      { name: "SAP ABAP", type: "sap", path: "sap-abap-training-chennai" },
      { path: "sap-ps", type: "sap", name: "SAP PS" },
      {  path: "sap-fica-online-training-chennai", type: "sap", name: "SAP FICA" },
      { name: "SAP ui5 fiori", type: "sap", path: "sap-ui5-fiori-training-chennai", },
      { name: "SAP BPC", type: "sap", path: "sap-bpc" },
      { name: "SAP SCM", type: "sap", path: "sap-scm" },
      { name: "SAP Transportation Management", type: "sap", path: "sap-transportation-management-training-chennai" },
      { name: "SAP SuccessFactors Online Training", type: "sap", path: "sap-successfactors-online-training-chennai" },
      { name: "SAP Quality Management (QM)", type: "sap", path: "sap-quality-management-(qm)-training-chennai" },
      { name: "SAP Ariba Training", type: "sap", path: "sap-ariba-training-chennai" },
      { name: "SAP Master Data Governance (MDG) on S/4HANA", type: "sap", path: "sap-master-data-governance-(mdg)-on-s-4hana-training-chennai" },
      { name: "Adobe Photoshop Certification", type: "sap" },
      { name: "SAP BASIS Administration", type: "sap", path: "sap-basis-administration-training-chennai" },
      { name: "SAP Sales Cloud (C4C) Training", type: "sap", path: "sap-sales-cloud-(c4c)-training" },
      { name: "SAP FSCM (Retail) Training", type: "sap", path: "sap-fscm" },
      { name: "SAP Business Technology Platform (BTP)", type: "sap", path: "sap-btp-training-chennai" },
      { name: "SAP CRM Training", type: "sap", path: "sap-crm-training-chennai" },
      { name: "SAP SRM Training", type: "sap", path: "sap-srm" },
      { name: "SAP APO Online Training", type: "sap", path: "sap-apo" },
      { name: "SAP IBP Online Training", type: "sap", path: "sap-ibp" },
      { name: "SAP BI/BW Training", type: "sap", path: "sap-bibw-training-chennai" },
      { name: "SAP Analytics Cloud Online Training", type: "sap", path: "sap-analytic-cloud-training-chennai" },
      { name: "SAP Security GRC Training", type: "sap", path: "sap-security-grc-training-chennai" },
      { name: "SAP IS Retail Training", type: "sap", path: "sap-is-retail" },
      { name: "SAP IS Utilities Training", type: "sap", path: "sap-is-utilities-training-chennai" },
      { name: "SAP PLM Training", type: "sap", path: "sap-plm" },
      { name: "SAP LE Certification Training", type: "sap", path: "sap-le" },
      { name: "SAP Healthcare Training", type: "sap", path: "sap-healthcare-training-chennai" },
      { name: "SAP IS Banking Training", type: "sap", path: "sap-is-banking" },
      { name: "SAP BPC Online Training", type: "sap", path: "sap-is-bpc" },
      { name: "SAP PM Course", type: "sap", path: "sap-pm-training-chennai" },
      { name: "SAP ESS Online", type: "sap", path: "sap-ess" },
      { name: "SAP Hybris E-Commerce Online Training", type: "sap", path: "sap-hybris-ecommerce-training-chennai" },
      { name: "SAP BO", type: "sap", path: "sap-bo-training-chennai" },
      { name: "SAP BOBJ Training", type: "sap", path: "sap-bobj" },
      { name: "SAP Fieldglass", type: "sap", path: "sap-fieldglass" },
      { name: "SAP Data Archiving", type: "sap", path: "sap-data-archiving" },
      { name: "SAP Solution Manager", type: "sap", path: "sap-solution-manager" },
      { name: "SAP Aerospace & Defense", type: "sap", path: "sap-aerospace-and-defense" },
      { name: " SAP IS Insurance Training", type: "sap", path: "sap-is-insurance" },

      // Oracle Courses
      { name: "Oracle Financials", type: "oracle", path: "oracle-financials-chennai" },
      { name: "Oracle SCM", type: "oracle", path: "oracle-scm-chennai" },
      { name: "Oracle HCM", type: "oracle", path: "oracle-hcm-chennai" },
      { name: "Oracle CPQ Training", type: "oracle", path: "oracle-cpq-chennai" },
      { name: "Oracle Procure to Pay Training", type: "oracle", path: "oracle-procure-to-pay" },
      { name: "Oracle Hyperion EPM Training", type: "oracle", path: "oracle-hyperion-epm" },
      { name: "Oracle GRC Controls Suite Fundamentals", type: "oracle", path: "oracle-grc-control-suit-fundamentals-vers" },
      { name: "Oracle Developer Course", type: "oracle", path: "oracle-developer" },
      { name: "Oracle SQL & PL/SQL", type: "oracle", path: "oracle-sql-plsql-chennai" },
      { name: "Oracle Forms and Reports", type: "oracle", path: "oracle-forms-and-reports" },
      { name: "Oracle Application Framework (OAF)", type: "oracle", path: "oracle-application-framework" },
      { name: "Oracle Cloud Infrastructure (OCI)", type: "oracle", path: "oracle-cloud-infrastructure" },
      { name: "WebLogic Server", type: "oracle", path: "oracle-weblogic-server" },
      { name: "Oracle PeopleSoft HRMS/HCM", type: "oracle", path: "oracle-peoplesoft-chennai" },
      { name: "Oracle Customer Care & Billing (CC&B)", type: "oracle", path: "oracle-customercare" },
      { name: "R12.x Oracle Inventory Management", type: "oracle", path: "oracle-r12" },
      { name: "R12.2 Oracle E-Business Tax", type: "oracle", path: "oracle-tax-reporting" },
      { name: "R12.x Oracle Order Management", type: "oracle", path: "oracle-order-management" }
    ];

    setAllCourses(courses);
  }, []);

  const handleHomeClick = () => {
    router.push("/");
  };

  const handleAboutClick = () => {
    router.push("/freelance-trainee");
  };

  const handleCall = () => {
    window.location.href = `tel:${phoneNumber}`;
  };

  const normalizeText = (text) => {
    return text.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, ' ').trim();
  };

  const calculateMatchScore = (courseName, searchQuery) => {
    const normalizedCourse = normalizeText(courseName);
    const normalizedQuery = normalizeText(searchQuery);
    const courseWords = normalizedCourse.split(' ');
    const queryWords = normalizedQuery.split(' ');

    if (normalizedCourse === normalizedQuery) return 1000;
    if (normalizedCourse.startsWith(normalizedQuery)) return 900;
    if (courseWords.some(word => word === normalizedQuery)) return 800;
    if (courseWords.some(word => word.startsWith(normalizedQuery))) return 700;
    
    const matchingWords = queryWords.filter(qWord => 
      courseWords.some(cWord => cWord.includes(qWord))
    ).length;
    if (matchingWords > 0) return 600 + (matchingWords * 50);
    
    if (normalizedCourse.includes(normalizedQuery)) return 500;
    
    return 0;
  };

  const findBestMatch = (input) => {
    const normalizedInput = normalizeText(input);
    const scoredCourses = allCourses
      .map(course => ({
        course,
        score: calculateMatchScore(course.name, normalizedInput)
      }))
      .filter(item => item.score > 0)
      .sort((a, b) => b.score - a.score);

    return scoredCourses.length > 0 ? scoredCourses[0].course : null;
  };

  const navigateToCourse = (course) => {
    const navigate = () => {
      if (course.type === "sap") {
        const urlPath = course.path || encodeURIComponent(course.name);
        router.push(`/courses/${urlPath}`);
      } else if (course.type === "oracle") {
        const urlPath = course.path || encodeURIComponent(course.name);
        router.push(`/courses/${urlPath}`);
      } else {
        router.push(`/courses/${encodeURIComponent(course.path)}`);
      }
      setSearchTerm("");
      setShowSuggestions(false);
      setActiveSuggestion(-1);
    };

    const isMobile = window.innerWidth <= 767;
    if (isMobile) {
      setTimeout(navigate, 200);
    } else {
      navigate();
    }
  };

  const handleSearch = (e) => {
    e.preventDefault();
    if (searchTerm.trim()) {
      // If there's an active suggestion (from keyboard navigation), use that
      if (activeSuggestion >= 0 && suggestions[activeSuggestion]) {
        navigateToCourse(suggestions[activeSuggestion]);
      } else {
        // Otherwise try to find the best match
        const bestMatch = findBestMatch(searchTerm.trim());
        if (bestMatch) {
          navigateToCourse(bestMatch);
        } else {
          alert("Course not found. Please try another search term.");
        }
      }
    }
  };

  const handleSuggestionClick = (course) => {
    // Focus the input first to prevent immediate blur
    searchInputRef.current?.focus();
    navigateToCourse(course);
  };

  const handleInputChange = (e) => {
    const value = e.target.value;
    setSearchTerm(value);
    setActiveSuggestion(-1);

    if (debounceTimerRef.current) {
      clearTimeout(debounceTimerRef.current);
    }

    if (value.trim().length > 0) {
      debounceTimerRef.current = setTimeout(() => {
        const normalizedInput = normalizeText(value);
        const scoredResults = allCourses
          .map(course => ({
            course,
            score: calculateMatchScore(course.name, normalizedInput)
          }))
          .filter(item => item.score > 0)
          .sort((a, b) => b.score - a.score)
          .slice(0, 10)
          .map(item => item.course);

        setSuggestions(scoredResults);
        setShowSuggestions(scoredResults.length > 0);
      }, 150);
    } else {
      setSuggestions([]);
      setShowSuggestions(false);
    }
  };

  const handleInputBlur = () => {
    // Only hide suggestions if the blur wasn't caused by clicking a suggestion
    setTimeout(() => {
      const activeElement = document.activeElement;
      if (!suggestionsRef.current?.contains(activeElement)) {
        setShowSuggestions(false);
      }
    }, 200);
  };

  const handleKeyDown = (e) => {
    // Arrow down - move to next suggestion
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setActiveSuggestion((prev) =>
        prev < suggestions.length - 1 ? prev + 1 : prev,
      );
    }
    // Arrow up - move to previous suggestion
    else if (e.key === "ArrowUp") {
      e.preventDefault();
      setActiveSuggestion((prev) => (prev > 0 ? prev - 1 : -1));
    }
    // Enter - select the active suggestion or submit search
    else if (e.key === "Enter") {
      if (activeSuggestion >= 0 && suggestions[activeSuggestion]) {
        e.preventDefault();
        navigateToCourse(suggestions[activeSuggestion]);
      }
    }
  };

  // Scroll active suggestion into view
  useEffect(() => {
    if (activeSuggestion >= 0 && suggestionsRef.current) {
      const activeItem = suggestionsRef.current.children[activeSuggestion];
      if (activeItem) {
        activeItem.scrollIntoView({
          block: "nearest",
          behavior: "smooth",
        });
      }
    }
  }, [activeSuggestion]);

  return (
    <>
      <nav className="appbar">
        {/* Logo */}
        <div className="logo-shine-wrapper">
          <img
            src="/assets/logo/logo2.png"
            alt="ICLP Logo"
            className="appbar-logo"
          />
          <span className="logo-shine"></span>
        </div>


        {/* Title (Now comes first) */}
        {/* Title with Subtitle */}
        <div className="title-container" onClick={handleHomeClick}>
          <p className="headingText">ICLP</p>
          <p className="subtitle">Upgrade to New Era</p>
        </div>

        {/* Search Bar with Suggestions (Now placed after the title) */}
        <div className="search-container">
          <form className="appbar-search" onSubmit={handleSearch}>
            {/* Your search input element goes here */}
            <input
              ref={searchInputRef}
              type="text"
              placeholder="Search courses..."
              value={searchTerm}
              onChange={handleInputChange}
              onFocus={() => searchTerm.length > 0 && setShowSuggestions(true)}
              onBlur={handleInputBlur}
              onKeyDown={handleKeyDown}
              className="search-input-dark"
              autoComplete="off"
              onTouchStart={() =>
                searchTerm.length > 0 && setShowSuggestions(true)
              }
            />
            <button type="submit" className="search-button">
              <FaSearch className="search-icon" />
            </button>
          </form>

          {/* REPLACE THE EXISTING SUGGESTIONS CODE WITH THE NEW VERSION BELOW */}
          {showSuggestions && suggestions.length > 0 && (
            <ul className="suggestions-dropdown" ref={suggestionsRef}>
              {suggestions.map((course, index) => (
                <li
                  key={index}
                  onPointerDown={(e) => {
                    e.preventDefault();
                    handleSuggestionClick(course);
                  }}
                  className={
                    index === activeSuggestion ? "active-suggestion" : ""
                  }
                >
                  {course.name}
                </li>
              ))}
            </ul>
          )}
        </div>

        {/* Button Container */}
        <div className="button-container" style={showSuggestions ? { pointerEvents: 'none' } : {}}>
          {/* About Button */}
          <button className="about-button" onClick={handleAboutClick}>
            <FaInfoCircle className="about-icon" /> Freelance Trainer
          </button>

          {/* Enquiry Button */}
          <button className="enquiry-button" onClick={handleCall}>
            <FaPhone className="phone-icon" /> Enquiry 86810 26181
          </button>

          {/* Social Media Icons */}
          <div className="social-icons">
            <a
              href={socialLinks.instagram}
              target="_blank"
              rel="noopener noreferrer"
            >
              <FaInstagram className="social-icon" />
            </a>
            <a
              href={socialLinks.linkedin}
              target="_blank"
              rel="noopener noreferrer"
            >
              <FaLinkedin className="social-icon" />
            </a>
            <a
              href={socialLinks.youTybe}
              target="_blank"
              rel="noopener noreferrer"
            >
              <FaYoutube className="social-icon" />
            </a>
          </div>
        </div>
      </nav>

      {/* Enhanced Marquee Banner */}
      <div className="dynamic-banner">
        <div className="dynamic-banner__track">
          <div className="dynamic-banner__content">
            <span className="dynamic-banner__text">
              We are hiring{" "}
              <span className="highlight-text">passionate trainers</span> to
              join our growing team! Share your{" "}
              <span className="highlight-text">expertise</span> with aspiring
              professionals in a{" "}
              <span className="highlight-text">
                dynamic learning environment
              </span>
              . Enjoy{" "}
              <span className="highlight-text">competitive compensation</span>,{" "}
              <span className="highlight-text">flexible schedules</span>, and
              rewarding teaching experiences. Whether you're a{" "}
              <span className="highlight-text">seasoned instructor</span> or{" "}
              <span className="highlight-text">industry expert</span>, we
              provide the platform to{" "}
              <span className="highlight-text">shape futures</span>.{" "}
              <span className="highlight-text">Apply now</span> to become part
              of our <span className="highlight-text">elite training team</span>
              !
            </span>
          </div>
        </div>
      </div>
      {/* <HolidayMarquee /> */}
    </>
  );
};

export default AppBar;