...
const router = useRouter();
  ...
  const BookParticle = ({ size, left, top, delay, xDir, duration, repeatDelay }) => {
    return (
      <motion.div
  ...
  // Precompute particle data - client only to avoid hydration mismatch
  const [particleData, setParticleData] = useState([]);
  useEffect(() => {
    setParticleData([...Array(10)].map(() => ({
      size: 20 + Math.random() * 20,
      left: Math.random() * 100,
      top: Math.random() * 100,
      delay: Math.random() * 3,
      xDir: Math.random() > 0.5 ? 20 : -20,
